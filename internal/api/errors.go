...
func NewAPIErrorWithReason(code int, message, reasonCode, reason string) *APIError {
	// Enhance message with friendly description if available
	if friendlyMsg, exists := errorCodeMap[code]; exists {
		message = fmt.Sprintf("%s: %s", friendlyMsg, message)
	}
